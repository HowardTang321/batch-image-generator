...
# =========================
# Etsy 浏览器采集 JSON 转 Excel
# =========================

ETSY_BROWSER_COLLECTOR_JS = r'''
(async function () {
  const config = { maxPages: 5, maxItems: 80, imagesPerProduct: 12, delayMs: 2500 };
  const sleep = (ms) => new Promise(r => setTimeout(r, ms));
  function cleanUrl(url) { if (!url) return ""; url = String(url).trim().replaceAll("\\u002F", "/").replaceAll("\\/", "/").replaceAll("&amp;", "&"); if (url.startsWith("//")) url = "https:" + url; if (url.startsWith("/listing/")) url = "https://www.etsy.com" + url; return url; }
  function stripListingUrl(url) { url = cleanUrl(url); try { const u = new URL(url); const m = u.pathname.match(/\/listing\/\d+\/[^/?#]+/) || u.pathname.match(/\/listing\/\d+/); if (m) return "https://www.etsy.com" + m[0]; } catch(e) {} return url.split("?")[0].split("#")[0]; }
  function makeShopPageUrl(shopUrl, page) { const u = new URL(shopUrl); u.searchParams.set("page", String(page)); return u.toString(); }
  async function fetchHtml(url) { const res = await fetch(url, {method:"GET", credentials:"include", headers:{"accept":"text/html,application/xhtml+xml,application/xml;q=0.9,*/*;q=0.8"}}); if (!res.ok) throw new Error(`请求失败：HTTP ${res.status}，URL：${url}`); const html = await res.text(); const lower = html.toLowerCase(); if (lower.includes("captcha") || lower.includes("are you a human")) throw new Error("疑似触发 Etsy 验证页面，请停止采集，降低频率后再试。"); return html; }
  const unique = (arr) => Array.from(new Set(arr.filter(Boolean)));
  function extractListingUrlsFromHtml(html) { const urls = []; const patterns = [/https:\/\/www\.etsy\.com\/listing\/\d+\/[^"'\\\s<>]+/g, /https:\\\/\\\/www\.etsy\.com\\\/listing\\\/\d+\\\/[^"'\\\s<>]+/g, /href="(\/listing\/\d+\/[^"]+)"/g]; for (const p of patterns) { let m; while ((m = p.exec(html)) !== null) { let raw = cleanUrl(m[1] || m[0]); if (raw.startsWith("/listing/")) raw = "https://www.etsy.com" + raw; if (raw.includes("/listing/")) urls.push(stripListingUrl(raw)); } } return unique(urls); }
  function decodeHtml(text) { const t = document.createElement("textarea"); t.innerHTML = text || ""; return t.value; }
  function extractTitle(html) { let m = html.match(/<meta[^>]+property=["']og:title["'][^>]+content=["']([^"']+)["']/i); if (m) return decodeHtml(m[1]); m = html.match(/<title[^>]*>([^<]+)<\/title>/i); if (m) return decodeHtml(m[1]).replace(" | Etsy", "").trim(); m = html.match(/"title"\s*:\s*"([^"]+)"/i); if (m) return decodeHtml(m[1]); return ""; }
  function extractImages(html) { const images = []; const patterns = [/https:\/\/i\.etsystatic\.com\/[^"'\\\s<>]+/g, /https:\\\/\\\/i\.etsystatic\.com\\\/[^"'\\\s<>]+/g]; for (const p of patterns) { let m; while ((m = p.exec(html)) !== null) { const url = cleanUrl(m[0]); if (url.includes("i.etsystatic.com")) images.push(url); } } return unique(images); }
  function extractVideos(html) { const videos = []; const patterns = [/https:\/\/v\.etsystatic\.com\/[^"'\\\s<>]+/g, /https:\/\/video\.etsystatic\.com\/[^"'\\\s<>]+/g, /https:\\\/\\\/v\.etsystatic\.com\\\/[^"'\\\s<>]+/g, /https:\\\/\\\/video\.etsystatic\.com\\\/[^"'\\\s<>]+/g, /https:\/\/[^"'\\\s<>]+\.mp4[^"'\\\s<>]*/g, /https:\\\/\\\/[^"'\\\s<>]+\.mp4[^"'\\\s<>]*/g]; for (const p of patterns) { let m; while ((m = p.exec(html)) !== null) { const url = cleanUrl(m[0]); if (url.includes("etsystatic.com") || url.includes(".mp4")) videos.push(url); } } return unique(videos); }
  async function collectShopListingUrls(shopUrl) { const all = []; const seen = new Set(); for (let page=1; page<=config.maxPages; page++) { const pageUrl = makeShopPageUrl(shopUrl, page); console.log(`读取店铺第 ${page} 页：`, pageUrl); const html = await fetchHtml(pageUrl); const urls = extractListingUrlsFromHtml(html); let newCount = 0; for (const url of urls) { if (!seen.has(url)) { seen.add(url); all.push(url); newCount++; if (all.length >= config.maxItems) return all; } } console.log(`第 ${page} 页找到 ${urls.length} 个链接，新增 ${newCount} 个。`); if (newCount === 0 && page > 1) break; await sleep(config.delayMs); } return all; }
  async function collectListingAssets(listingUrl, index) { console.log(`采集商品 ${index}:`, listingUrl); const html = await fetchHtml(listingUrl); let images = extractImages(html); let videos = extractVideos(html); if (config.imagesPerProduct > 0) images = images.slice(0, config.imagesPerProduct); return { index, title: extractTitle(html), listing_url: listingUrl, main_image: images[0] || "", images, videos, status: "成功", error: "" }; }
  ...
  try { let shopUrl = window.location.href; if (!shopUrl.includes("etsy.com/shop/")) shopUrl = prompt("请输入 Etsy 店铺链接：", "https://www.etsy.com/shop/HappyLaceCo"); if (!shopUrl) return; const listingUrls = await collectShopListingUrls(shopUrl); const results = []; for (let i=0; i<listingUrls.length; i++) { try { results.push(await collectListingAssets(listingUrls[i], i+1)); } catch(e) { results.push({index:i+1,title:"",listing_url:listingUrls[i],main_image:"",images:[],videos:[],status:"失败",error:String(e.message || e)}); } await sleep(config.delayMs); } const output = {source:"etsy_browser_collector", shop_url:shopUrl, collected_at:new Date().toISOString(), config, total:results.length, results}; console.log("采集完成：", output); downloadJson(output, "etsy_shop_assets.json"); alert(`采集完成，共 ${results.length} 个商品。JSON 文件已下载。`); } catch(e) { console.error(e); alert("采集失败：" + String(e.message || e)); }
})();
'''.strip()
...
def build_etsy_boss_rows_from_json(data: dict):
    ...
    for item in data.get("results", []):
        idx = item.get("index", "")
        rows.append({"编号": f"{int(idx):03d}" if str(idx).isdigit() else idx, "商品名称": item.get("title", ""), "类型": "商品链接", "序号": "", "链接": item.get("listing_url", ""), "状态": item.get("status", ""), "错误信息": item.get("error", "")})
        for i, url in enumerate(item.get("images", []) or [], start=1):
            rows.append({"编号": "", "商品名称": "", "类型": "图片", "序号": i, "链接": url, "状态": "", "错误信息": ""})
        for i, url in enumerate(item.get("videos", []) or [], start=1):
            rows.append({"编号": "", "商品名称": "", "类型": "视频", "序号": i, "链接": url, "状态": "", "错误信息": ""})
        rows.append({"编号": "", "商品名称": "", "类型": "", "序号": "", "链接": "", "状态": "", "错误信息": ""})
    return rows
...
def convert_ehunt_df(df, images_per_product=10):
    title_col = find_column(df, ["产品名称", "商品名称", "标题", "Title", "Product Title", "Product Name", "Listing Title", "Name"])
    listing_url_col = find_column(df, ["商品链接", "产品链接", "Listing URL", "Listing Link", "Product URL", "Product Link", "URL", "Link"])
    main_image_col = find_column(df, ["主图", "主图链接", "Main Image", "Main Image URL", "Image", "Image URL", "Thumbnail", "Photo"])
    images_col = find_column(df, ["全部图片", "图片链接", "图片", "Images", "Image URLs", "All Images", "Product Images", "Photos"])
    video_col = find_column(df, ["视频", "视频链接", "Videos", "Video", "Video URL", "Video URLs", "Product Videos"])
    shop_col = find_column(df, ["店铺", "店铺名称", "Shop", "Shop Name", "Store", "Store Name"])
    price_col = find_column(df, ["价格", "Price", "Sale Price"])
    sales_col = find_column(df, ["销量", "总销量", "Total Sales", "Sales", "Sale Count"])
    favorites_col = find_column(df, ["收藏", "收藏数", "Favorites", "Favorite Count", "Favourites"])
    ...
    for _, row in df.iterrows():
        ...
        if main_image_col:
            for img in reversed(split_links(row.get(main_image_col, ""))):
                if img not in images:
                    images.insert(0, img)
        images = [x for x in images if x.startswith("http")]
        images = list(dict.fromkeys(images))[:int(images_per_product)]
        videos = []
        if video_col:
            videos = list(dict.fromkeys([x for x in split_links(row.get(video_col, "")) if x.startswith("http")]))
...
def build_material_boss_rows(results):
    rows = []
    for item in results:
        idx = item.get("index", "")
        rows.append({
            ...
            "类型": "商品链接",
            "序号": "",
            "链接": item.get("listing_url", ""),
            "状态": item.get("status", ""),
            "错误信息": item.get("error", ""),
        })
        for image_index, image_url in enumerate(item.get("images", []), start=1):
            rows.append({"编号": "", "商品名称": "", "店铺名称": "", "价格": "", "销量": "", "收藏": "", "类型": "图片", "序号": image_index, "链接": image_url, "状态": "", "错误信息": ""})
        for video_index, video_url in enumerate(item.get("videos", []), start=1):
            rows.append({"编号": "", "商品名称": "", "店铺名称": "", "价格": "", "销量": "", "收藏": "", "类型": "视频", "序号": video_index, "链接": video_url, "状态": "", "错误信息": ""})
        rows.append({"编号": "", "商品名称": "", "店铺名称": "", "价格": "", "销量": "", "收藏": "", "类型": "", "序号": "", "链接": "", "状态": "", "错误信息": ""})
    return rows


def build_material_ai_rows(results):
    rows = []
    for item in results:
        idx = item.get("index", 0)
        images = item.get("images", []) or []
...
with st.sidebar:
    page = st.radio(
        "选择功能",
        ["AI 批量生图", "Etsy 素材采集结果导入", "eHunt 数据导入", "eHunt 店铺查询"],
        index=0,
    )
    st.divider()
...
if page == "Etsy 素材采集结果导入":
    st.subheader("🛒 Etsy 店铺素材采集结果导入")
    st.info("如果 Streamlit 云服务器直接请求 Etsy 返回 403，可以在浏览器里运行采集脚本，下载 JSON 后上传到这里生成 Excel。")

    with st.expander("第一步：下载 / 复制浏览器采集脚本"):
        st.download_button(
            label="下载 etsy_browser_collector.js",
            data=ETSY_BROWSER_COLLECTOR_JS,
            file_name="etsy_browser_collector.js",
            mime="text/javascript",
        )
        st.code(ETSY_BROWSER_COLLECTOR_JS[:2500] + "\n\n...脚本较长，建议直接下载。", language="javascript")
        st.write("使用方式：打开 Etsy 店铺页面 → F12 → Console → 粘贴脚本 → 回车 → 下载 etsy_shop_assets.json。")

    uploaded_json = st.file_uploader("第二步：上传 Etsy 采集 JSON", type=["json"])
    if uploaded_json:
        try:
        ...
        boss_df = pd.DataFrame(build_etsy_boss_rows_from_json(data))
        ai_df = pd.DataFrame(build_etsy_ai_rows_from_json(data))
        total = len(data.get("results", []))
        success_count = sum(1 for x in data.get("results", []) if x.get("status") == "成功")

        st.success(f"解析完成：共 {total} 个商品，成功 {success_count} 个。")
        st.subheader("老板查看表")
        st.dataframe(boss_df, use_container_width=True)
        st.subheader("AI 生图导入表")
        st.dataframe(ai_df, use_container_width=True)

        excel_bytes = make_excel_download({"老板查看表": boss_df, "AI生图导入表": ai_df})
        st.download_button(
...
if page == "eHunt 数据导入":
    ...
    st.info("推荐流程：先在 eHunt 导出 CSV / Excel，再上传到这里整理成老板查看表和 AI 生图导入表。")

    uploaded_ehunt_file = st.file_uploader("上传 eHunt 导出的 CSV / Excel", type=["csv", "xlsx", "xls"])
    col1, col2 = st.columns(2)
    with col1:
        images_per_product = st.number_input("每个商品最多保留图片数", min_value=1, max_value=30, value=10, step=1)
    with col2:
    ...
    if uploaded_ehunt_file:
        ...
        results = convert_ehunt_df(raw_df, images_per_product=int(images_per_product))
        if not results:
            st.error("没有识别到有效商品数据。请检查导出表是否包含商品标题、商品链接、图片链接等字段。")
            st.stop()

        boss_df = pd.DataFrame(build_material_boss_rows(results))
        ai_df = pd.DataFrame(build_material_ai_rows(results))
        st.subheader("老板查看表")
        st.dataframe(boss_df, use_container_width=True)
        st.subheader("AI 生图导入表")
        st.dataframe(ai_df, use_container_width=True)
