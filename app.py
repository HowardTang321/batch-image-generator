...
# 阿里云 OSS 配置，可选；不配置时只在网页和 ZIP 里保存结果，不上传 OSS。
...
def make_oss_object_key(timestamp: str, filename: str) -> str:
    """生成 OSS 保存路径：generated-images/日期/batch_时间/文件名"""
    date_part = datetime.now().strftime("%Y-%m-%d")
    return f"generated-images/{date_part}/batch_{timestamp}/{safe_filename(filename)}"


def upload_bytes_to_aliyun_oss(
    file_bytes: bytes,
    object_key: str,
...
def get_headers() -> dict:
    if not APIMART_API_KEY:
        raise ValueError("没有检测到 APIMART_API_KEY，请先在 Streamlit Secrets 或 .env 文件中配置。")
...
def poll_task_result(
    ...
    """
    轮询任务状态，返回图片 URL 列表。
    增强版：遇到 SSL / 网络抖动时自动重试，避免单次查询失败导致整张图失败。
    """
    ...
    while True:
        ...
        try:
            ...
            try:
                ...
                raise RuntimeError(
                    f"查询任务失败，HTTP {response.status_code}，响应内容：{response.text}"
                )

            if response.status_code != 200:
                raise RuntimeError(
                    f"查询任务失败，HTTP {response.status_code}，响应内容：{data}"
                )
            ...
            if consecutive_errors > max_query_retries:
                raise RuntimeError(
                    f"查询任务时网络连接多次失败，已超过最大重试次数。"
                    f"task_id：{task_id}，错误：{e}"
                )

            wait_seconds = interval_seconds * consecutive_errors
...
with st.sidebar:
    ...
    st.write("OSS 状态：")
    if oss2 is None:
        st.warning("未安装 oss2，无法上传阿里云 OSS")
    elif is_oss_configured():
        st.success("已检测到阿里云 OSS 配置")
    else:
        st.info("未配置 OSS，将只生成 ZIP，不上传云端")
...
if not APIMART_API_KEY:
    st.error("请先在 Streamlit Secrets 或 .env 文件中配置 APIMART_API_KEY。")
    st.stop()
    ...
    for future in as_completed(future_map):
        ...
        try:
            ...
            for img in result["images"]:
                ...
                oss_url = upload_bytes_to_aliyun_oss(
                    img["image_bytes"],
                    object_key,
